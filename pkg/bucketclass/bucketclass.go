...
func CmdCreate() *cobra.Command {
	...
	cmd.Flags().StringSlice("backingstores", nil,
		"Set first tier backing stores (use commas or multiple flags)/n"+
			"for placemant type NS please first provide write target and then read targets")
	// cmd.AddCommand(
	// 	CmdNSCreate(),
	// )
	return cmd
...
// CmdCreate returns a CLI command
func CmdNSCreate() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "namespace-policy",
		Short: "Create bucket class",
		Run:   RunCreate,
	}
