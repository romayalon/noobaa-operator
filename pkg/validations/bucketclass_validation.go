...
func ValidateBucketClass(bc *nbv1.BucketClass) error {
	...
	if bc.Spec.NamespacePolicy != nil {
		if err := ValidateNamespacePolicy(bc.Spec.NamespacePolicy, bc.Namespace); err != nil {
			return err
		}
	}
	if bc.Spec.PlacementPolicy != nil {
		if err := ValidatePlacementPolicy(bc.Spec.PlacementPolicy, bc.Namespace); err != nil {
			return err
		}
	}
...
func ValidateTiersNumber(tiers []nbv1.Tier) error {
	if len(tiers) != 1 && len(tiers) != 2 {
		return util.NewPersistentError("TooManyTiers",
			fmt.Sprintf("unsupported number of tiers, bucketclass supports only 1 or 2 tiers, found %d", len(tiers)))
	}
	return nil
}
...
func ValidatePlacementPolicy(placementPolicy *nbv1.PlacementPolicy, namespace string) error {
	...
	if err := ValidateTiersNumber(placementPolicy.Tiers); err != nil {
		return err
	}
...
func ValidateNamespacePolicy(namespacePolicy *nbv1.NamespacePolicy, namespace string) error {
	...
	for _, name := range namespaceStoresArr {
		...
		if nsStore.Spec.Type == nbv1.NSStoreTypeNSFS && namespacePolicy.Type != nbv1.NSBucketClassTypeSingle {
			return util.NewPersistentError("InvalidNamespaceStoreType",
				fmt.Sprintf("invalid namespaceStore types, nsfs namespacestore %q is allowed on bucketclass of type single", name),
			)
		}
