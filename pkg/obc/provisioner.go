...
func (r *BucketRequest) CreateBucket() error {
	...
	if r.BucketClass.Spec.NamespacePolicy.WriteResource != "" {
		...
		log.Infof("RES_ROM1: write: %+v, Read: %+v, TTL %+v, Prefix: %+v", writeResource, readResources, cacheTTL, cachePrefix)
		createBucketParams.Namespace = &nb.NamespaceBucketInfo{
			WriteResource: writeResource,
			ReadResources: readResources,
			Caching:       nb.CacheSpec{TTLMs: cacheTTL, Prefix: cachePrefix, Fetched: false}, // here in the future need to add the cache prefix
		}
	}
